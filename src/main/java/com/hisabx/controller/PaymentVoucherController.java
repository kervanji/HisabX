...
public class PaymentVoucherController {
    ...
    private void updateCustomerBalance() {
        ...
        if (customer != null) {
            ...
            descriptionArea.setText("دفع إلى حساب .. " + customer.getName());
        }
        updateCurrentBalance();
    }
    ...
    private void handleNew() {
        currentVoucher = null;
        long nextNumber = voucherService.getNextPaymentVoucherNumber();
    ...
    private void handleSave() {
        ...
        try {
            ...
            Voucher savedVoucher = voucherService.createPaymentVoucher(voucher);
            
            showSuccess("تم الحفظ", "تم حفظ سند الدفع رقم: " + savedVoucher.getVoucherNumber());
            handleNew();
            
        } catch (Exception e) {
            logger.error("Failed to save payment voucher", e);
            showError("خطأ", "فشل في حفظ سند الدفع: " + e.getMessage());
        }
    }
    ...
    private void handlePreviousVouchers() {
        try {
            List<Voucher> vouchers = voucherService.getPaymentVouchers();
            
            StringBuilder sb = new StringBuilder();
            sb.append("السندات السابقة (سندات الدفع):\n\n");
            ...
            if (vouchers.isEmpty()) {
                sb.append("لا توجد سندات دفع سابقة");
            }
    ...
    private void handleCancel() {
        if (tabMode) {
            com.hisabx.util.TabManager.getInstance().closeTab("payment-voucher");
